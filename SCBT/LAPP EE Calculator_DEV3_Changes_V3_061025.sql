/**********************************************************************
FORMULA NAME: LAPP EE Calculator
...
4.0  	  Sendil		10-Jun-2025				LAPP Pension Enhancement
----------------------------------------------------------------------------------------------------------------------------------------------------
*****************************************************************************************************************************************************/
...
l_wsa_number=0
l_wsa_pension_hrs=0
l_assignment_id=GET_CONTEXT(PAYROLL_ASSIGNMENT_ID,0)
...
l_log = ESS_LOG_WRITE('LAPP_EE_PRESTAT_DEDN_CALCULATOR ASG l_payroll_rel_id--->'||to_char(l_payroll_rel_id))

/* Update LAPP EE Pension Enhancement START*/

CHANGE_CONTEXTS(PAYROLL_ASSIGNMENT_ID = l_assignment_id)
(
l_hr_asg_id = ASG_HR_ASG_ID
)
l_asg_primary_flag1 =  (GET_VALUE_SET('SC_PAY_LAPP_ASSIGNMENT_FLAG_VS'
			                                          ,'|=P_ASG_ID='''||TO_CHAR(l_hr_asg_id)||''''
												     ))

l_log = ESS_LOG_WRITE('LAPP_EE_PRESTAT_DEDN_CALCULATOR ASG l_hr_asg_id--->'||to_char(l_hr_asg_id)) 
l_log = ESS_LOG_WRITE('LAPP_EE_PRESTAT_DEDN_CALCULATOR ASG l_assignment_id--->'||l_asg_primary_flag1)

/* Update LAPP EE Pension Enhancement END*/
...
k = 1 
	WHILE (RUN_INCLUDED_PAYROLL_ASGS.EXISTS(k)) LOOP 
	( 
			
		CHANGE_CONTEXTS(PAYROLL_ASSIGNMENT_ID = RUN_INCLUDED_PAYROLL_ASGS[k])
		( 
			
			HR_ASGS[k] = ASG_HR_ASG_ID
...
if(l_asg_primary_flag1='Y') then( /* Update LAPP EE Pension Enhancement END*/
	l_override_flag 					 = 'Y'
 ...
 WHILE PER_HIST_ASG_ASSIGNMENT_ID.Exists(j)
    ...
    (   
		...
		/* Commented Old DBI's for Under and Over YMPE*/
			...
			ELSE /* Update LAPP EE Pension Enhancement START*/
			(
...
(
  ...
  l_comb_pensionable_earnings_ptd = LEAST (l_MIN_PERIODIC_PENSIONABLE_EARNINGS_1+l_LAPP_LTD_PTD+0, l_Pay_Check_Limit_period)   
