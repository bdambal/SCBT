...
1.4		  Sendil		12-Nov-2025		NA		 Included EE and ER Contribution changes based on APEX Under YMPE and Above YMPE Limit
...
(
  ...
  */
  ...
  dummy = ESS_LOG_WRITE(Formula_Name||' Amount_Accrued = '||TO_CHAR(Amount_Accrued))
   dummy = ESS_LOG_WRITE(Formula_Name||' retro_run = '||TO_CHAR(retro_run))
	  dummy = ESS_LOG_WRITE(Formula_Name||' retro_run = '||TO_CHAR(retro_er_run))					   
  dummy = ESS_LOG_WRITE(Formula_Name||' EE_ID = '||EE_ID)
/* V1.0 Begin*/
...
IF (
	...
	(	
	 ...
	 l_log = ESS_LOG_WRITE(Formula_Name|| ': APEX Pension Override Information Element Entry TRUE ' )
	 l_log = ESS_LOG_WRITE(Formula_Name|| ': l_override_EE : '||TO_CHAR(l_override_EE ))
	 l_log = ESS_LOG_WRITE(Formula_Name|| ': l_override_ER : '||TO_CHAR(l_override_ER ))
	 l_log = ESS_LOG_WRITE(Formula_Name|| ': l_override_cal_pen_earnings : '||TO_CHAR(l_override_cal_pen_earnings ))
	 l_log = ESS_LOG_WRITE(Formula_Name|| ': l_override_pen_service : '||TO_CHAR(l_override_pen_service ))
	 l_log = ESS_LOG_WRITE(Formula_Name|| ': l_override_pen_hours : '||TO_CHAR(l_override_pen_hours ))
	)
ELSE 
(
 	 l_log = ESS_LOG_WRITE(Formula_Name|| ': APEX Pension Override Information Element Entry FALSE ' )
...
i = 1 
	...
	( 
		...
		( 
			...
			l_Under_YMPE_EE_Rate = TO_NUMBER(GET_TABLE_VALUE('SCBT_PAY_FF_DEFAULTS', 'Rate','APEX UNDER YMPE EE RATE'))/100
			l_Above_YMPE_EE_Rate = TO_NUMBER(GET_TABLE_VALUE('SCBT_PAY_FF_DEFAULTS', 'Rate','APEX ABOVE YMPE EE RATE'))/100
			
			l_Under_YMPE_ER_Rate = TO_NUMBER(GET_TABLE_VALUE('SCBT_PAY_FF_DEFAULTS', 'Rate','APEX UNDER YMPE ER RATE'))/100
			l_Above_YMPE_ER_Rate = TO_NUMBER(GET_TABLE_VALUE('SCBT_PAY_FF_DEFAULTS', 'Rate','APEX ABOVE YMPE ER RATE'))/100
			...
			l_apex_YMPE_limit = TO_NUMBER(GET_TABLE_VALUE('SCBT_PAY_FF_DEFAULTS', 'Limit','APEX PENSION YMPE LIMIT'))
			...
			l_log=ESS_LOG_WRITE('L_SUP_PENSION_COMP_ASG_RUN :'    ||' '||TO_CHAR(L_SUP_PENSION_COMP_ASG_RUN))
			...
			l_apex_Under_YMPE_limit = l_apex_YMPE_limit/L_PAY_PERIODS_PER_YEAR /*APEX YMPE Limit - 75000*/
			l_apex_Above_YMPE_limit = l_cal_pension_earning - l_apex_Under_YMPE_limit
			
			l_log = ESS_LOG_WRITE(Formula_Name|| ' l_apex_Under_YMPE_limit_SK: ' || to_char(l_apex_Under_YMPE_limit))
			l_log = ESS_LOG_WRITE(Formula_Name|| ' l_apex_Over_YMPE_limit_SK: ' || to_char(l_apex_Above_YMPE_limit))
			...
			l_max_period_EE_con_limit_Under_YMPE = l_apex_Under_YMPE_limit* l_Under_YMPE_EE_Rate /*l_under_YMPE_ee_rate is 0.0278*/
            l_max_period_EE_con_limit_Above_YMPE = l_apex_Above_YMPE_limit * l_Above_YMPE_EE_Rate /*l_Above_YMPE_ee_rate is 0.0195*/
			l_max_period_EE_con_limit = l_max_period_EE_con_limit_Under_YMPE + l_max_period_EE_con_limit_Above_YMPE
		
			
			l_EE_contibution_Under_YMPE = l_apex_Under_YMPE_limit* l_Under_YMPE_EE_Rate /*l_under_YMPE_ee_rate is 0.0278*/
			l_EE_contibution_Above_YMPE = ((l_cal_pension_earning-l_apex_Under_YMPE_limit) * l_Above_YMPE_EE_Rate) /*l_Above_YMPE_ee_rate is 0.0195*/
			l_EE_contibution_Total = l_EE_contibution_Under_YMPE + l_EE_contibution_Above_YMPE
			l_EE_contibution = LEAST(l_EE_contibution_Total,l_max_period_EE_con_limit)
			...
			l_max_period_ER_con_limit_Under_YMPE = l_apex_Under_YMPE_limit* l_Under_YMPE_ER_Rate /*l_under_YMPE_er_rate is 0.0340*/
            l_max_period_ER_con_limit_Above_YMPE = l_apex_Above_YMPE_limit * l_Above_YMPE_ER_Rate /*l_Above_YMPE_er_rate is 0.0238*/
			l_max_period_ER_con_limit = l_max_period_ER_con_limit_Under_YMPE + l_max_period_ER_con_limit_Above_YMPE
		
			l_ER_contibution_Under_YMPE = l_apex_Under_YMPE_limit* l_Under_YMPE_ER_Rate /*l_under_YMPE_er_rate is 0.0340*/
			l_ER_contibution_Above_YMPE = ((l_cal_pension_earning-l_apex_Under_YMPE_limit) *  l_Above_YMPE_ER_Rate) /*l_Above_YMPE_er_rate is 0.0238*/
			l_ER_contibution_Total = l_ER_contibution_Under_YMPE + l_ER_contibution_Above_YMPE
			l_ER_contibution = LEAST(l_ER_contibution_Total,l_max_period_ER_con_limit)
			

			l_calculated_ER_contibution = l_ER_contibution
		   
			l_log = ESS_LOG_WRITE(Formula_Name|| ' l_calculated_ER_contibution: ' || to_char(l_ER_contibution))
		...
		l_log = ESS_LOG_WRITE(Formula_Name|| ' l_actual_EE_contibution: ' || to_char(l_EE_contibution))
		l_log = ESS_LOG_WRITE(Formula_Name|| ' l_actual_ER_contibution: ' || to_char(l_ER_contibution))
		l_log = ESS_LOG_WRITE(Formula_Name|| ' l_actual_pensionable_earnings: ' || to_char(CALCULATED_PENSIONABLE_EARNINGS))
			/* Prorate Pension earning based on actual er amount */	
			
			IF l_debug_flag = 'Y' THEN
			(
				l_log = ESS_LOG_WRITE(Formula_Name|| ' HR_ASGS[i]: ' || to_char(HR_ASGS[i]))
				l_log = ESS_LOG_WRITE(Formula_Name|| ' l_APEX_ASG_RUN: ' || to_char(l_APEX_ASG_RUN))
    ...
    /*CALL_FORMULA(
	 ...
	 CALL_FORMULA(
			...
			l_log = ESS_LOG_WRITE(Formula_Name|| ' l_SC_APEX_PENSION_SERVICE_REL_NOCB_PTD: ' || to_char(l_SC_APEX_PENSION_SERVICE_REL_NOCB_PTD))
			l_log = ESS_LOG_WRITE(Formula_Name|| ' l_SC_APEX_PENSIONABLE_HOURS_REL_NOCB_PTD: ' || to_char(l_SC_APEX_PENSIONABLE_HOURS_REL_NOCB_PTD))
			l_log = ESS_LOG_WRITE(Formula_Name|| ' l_asg_primary_flag: ' || (l_asg_primary_flag))
			l_log = ESS_LOG_WRITE(Formula_Name|| ' l_annual_normal_hours: ' || to_char(l_annual_normal_hours))													
			l_normal_hours=0			
            ...
            (
			   /*PENSION_SERVICE = ROUND((l_FTE/PAY_PERIODS_PER_YEAR),4)*/
				...
				l_log = ESS_LOG_WRITE(Formula_Name|| ' : APEX Pension Service Calculation TRUE : ')
			    l_log = ESS_LOG_WRITE(Formula_Name|| ' CALCULATED TOTAL_PENSION_SERVICE: ' || to_char(TOTAL_PENSION_SERVICE))				
			    l_log = ESS_LOG_WRITE(Formula_Name|| ' CALCULATED PENSION_SERVICE: ' || to_char(PENSION_SERVICE))
  ...
  /* Pension Service Calculation End*/	
    ...
    l_log=ESS_LOG_WRITE('l_ee_rate :'                                     ||' '||TO_CHAR(l_ee_rate))
    l_log=ESS_LOG_WRITE('l_apex_con_limit:'                               ||' '||TO_CHAR(l_apex_con_limit))
    l_log=ESS_LOG_WRITE('l_apex_period_limit :'                           ||' '||TO_CHAR(l_apex_period_limit))
    l_log=ESS_LOG_WRITE('L_SUP_PENSION_COMP_ASG_RUN :'                    ||' '||TO_CHAR(L_SUP_PENSION_COMP_ASG_RUN))
    l_log=ESS_LOG_WRITE('l_cal_pension_earning :'                         ||' '||TO_CHAR(l_cal_pension_earning))
    l_log=ESS_LOG_WRITE('l_max_period_EE_con_limit :'                     ||' '||TO_CHAR(l_max_period_EE_con_limit))
    l_log=ESS_LOG_WRITE('l_max_annual_EE_con_limit :'                     ||' '||TO_CHAR(l_max_annual_EE_con_limit))
    l_log=ESS_LOG_WRITE('l_EE_contibution :'                              ||' '||TO_CHAR(l_EE_contibution))
    l_log=ESS_LOG_WRITE('l_er_rate :'                                     ||' '||TO_CHAR(l_er_rate))
    l_log=ESS_LOG_WRITE('l_apex_con_limit :'                              ||' '||TO_CHAR(l_apex_con_limit))
    l_log=ESS_LOG_WRITE('l_max_period_ER_con_limit :'                     ||' '||TO_CHAR(l_max_period_ER_con_limit))
    l_log=ESS_LOG_WRITE('l_max_annual_ER_con_limit :'                     ||' '||TO_CHAR(l_max_annual_ER_con_limit))
    l_log=ESS_LOG_WRITE('l_ER_contibution : '                             ||' '||TO_CHAR(l_ER_contibution))
    l_log=ESS_LOG_WRITE(' Apex Payroll Rel Id :'                          ||' '||TO_CHAR(l_payroll_rel_id))
	l_log=ESS_LOG_WRITE(' Apex Payroll l_hr_term_id : '                   ||' '||TO_CHAR(l_hr_term_id))
	l_log=ESS_LOG_WRITE(' Apex Payroll hr_asg_id :'                       ||' '||TO_CHAR(hr_asg_id))
	l_log=ESS_LOG_WRITE(' Apex Payroll l_asg_primary_flag :'              ||' '||(l_asg_primary_flag))
	l_log=ESS_LOG_WRITE(' Apex Payroll l_FTE :'                           ||' '||TO_CHAR(l_FTE))
	l_log=ESS_LOG_WRITE(' Apex Payroll PENSION_SERVICE :'                 ||' '||TO_CHAR(PENSION_SERVICE))
	l_log=ESS_LOG_WRITE(' Apex Payroll PENSION_HOURS :'                 ||' '||TO_CHAR(PENSION_HOURS))
	l_log=ESS_LOG_WRITE(' Apex Payroll CALCULATED_PENSIONABLE_EARNINGS :' ||' '||TO_CHAR(CALCULATED_PENSIONABLE_EARNINGS))
	l_log=ESS_LOG_WRITE(' Apex Payroll l_APEX_SUP_PENSION_EE_REL_YTD :' ||' '||TO_CHAR(l_APEX_SUP_PENSION_EE_REL_YTD))
	l_log=ESS_LOG_WRITE(' Apex Payroll l_APEX_ER_CONT_REL_YTD :' ||' '||TO_CHAR(l_APEX_ER_CONT_REL_YTD))
	...
	(
		PENSION_SERVICE = APEX_SUP_PENSION_EE_PENSION_SERVICE_REL_ENTRY_VALUE
																...
																l_log=ESS_LOG_WRITE(' Apex Payroll Final_calc_earnings :' ||' '||TO_CHAR(CALCULATED_PENSIONABLE_EARNINGS))	
)
...
/* V1.0 End*/	
  ...
  */
  ...
  (
    ...
    RETURN Dedn_Amt, Stop_Entry, To_Total_Owed, Mesg, To_Arrears, Not_Taken,ER_CONTRIBUTION,PENSION_SERVICE,CALCULATED_PENSIONABLE_EARNINGS,PENSION_HOURS,l_apex_Under_YMPE_limit,l_apex_Above_YMPE_limit
  )
  ELSE
  (
    IF (Mesg <> 'ZZZ') THEN RETURN Dedn_Amt, To_Total_Owed, To_Arrears, Not_Taken, Mesg,ER_CONTRIBUTION,PENSION_SERVICE,CALCULATED_PENSIONABLE_EARNINGS,PENSION_HOURS,l_apex_Under_YMPE_limit,l_apex_Above_YMPE_limit
    ELSE RETURN Dedn_Amt, To_Total_Owed, To_Arrears, Not_Taken,ER_CONTRIBUTION,PENSION_SERVICE,CALCULATED_PENSIONABLE_EARNINGS,PENSION_HOURS,l_apex_Under_YMPE_limit,l_apex_Above_YMPE_limit
  )  
)
